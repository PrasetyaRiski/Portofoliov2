...
<div class="flex items-center justify-between mb-8">
    <div>
        ...
        <h2 class="text-2xl font-bold">Edit: {{ $certificate->title }}</h2>
    </div>
    
    @if($certificate->credential_url)
    <a href="{{ $certificate->credential_url }}" target="_blank" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
...
<form action="{{ route('admin.certificates.update', $certificate) }}" method="POST" enctype="multipart/form-data" x-data="certificateForm()">
    @csrf
    @method('PUT')
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="mb-6">
                    ...
                    >
                        ...
                        <option value="{{ $key }}" {{ old('category', $certificate->category) === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('category')
                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="mb-6">
                    ...
                    <div class="relative inline-block">
                        <img src="{{ $certificate->image_url }}" alt="{{ $certificate->title }}" class="max-h-48 rounded-lg">
                    ...
                    <template x-if="!imagePreview">
                        <div>
                            ...
                            <p class="text-gray-600 dark:text-gray-400 mb-2">
                                {{ $certificate->image ? 'Upload new image to replace' : 'Drag and drop or click to upload' }}
                            </p>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="mb-6">
                    ...
                    >
                        <option value="draft" {{ old('status', $certificate->status) === 'draft' ? 'selected' : '' }}>Draft</option>
                        <option value="published" {{ old('status', $certificate->status) === 'published' ? 'selected' : '' }}>Published</option>
                    </select>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-semibold">{{ $certificate->created_at->format('M d, Y') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-500 dark:text-gray-400">Last Updated</span>
                        <span class="font-semibold">{{ $certificate->updated_at->diffForHumans() }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-900 p-6">
                ...
                <p class="text-sm text-red-500 dark:text-red-400 mb-4">
                    Once deleted, this certificate cannot be recovered.
                </p>
                <button 
                    type="button"
                    x-data
                    @click="if(confirm('Are you sure you want to delete this certificate?')) { $refs.deleteForm.submit() }"
...
<form x-ref="deleteForm" action="{{ route('admin.certificates.destroy', $certificate) }}" method="POST" class="hidden">
    @csrf
    @method('DELETE')
</form>
@endsection
